...
def get_album_links(url, album_headers, f_path):
    ...
    with open(f_path+"AlbumInfo.txt", 'a', encoding='utf8') as f:
        for title in titles:
            ...
            # Todo: sre_constants.error: nothing to repeat at position 65
            try:
...
def get_all_singers_id(path="../result/all_singer.csv"):
    """
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            if len(line.split(",")) != 3:
                ...
                line = line.split(",")
                singer_id = line[0]
                singer_category = line[-1]
                singer_name = ",".join(line[1:-1])
                #print(singer_id,singer_name,singer_category)
            else:
                singer_id, singer_name,singer_category = line.split(",")
            singer_dict[singer_id]  = (singer_name, singer_category)
    return singer_dict

def get_singer_songs_info(f_path, singer_id=3686):
    ...
    album_headers = {
        ...
        'Cookie': '_iuqxldmzr_=32; _ntes_nnid=b9e9c8a460bfdbc1250ffc5908f95cad,1543554353478; _ntes_nuid=b9e9c8a460bfdbc1250ffc5908f95cad;\
         __utma=94650624.345999225.1543554354.1543554354.1543554354.1; __utmc=94650624; \
         __utmz=94650624.1543554354.1.1.utmcsr=baidu|utmccn=(organic)|utmcmd=organic; WM_TID=N3LQ47ihEYOKTieS18tLGKdN8q6R0iyt; \
         WM_NI=VJ7qPEylWXfPBYYl3aMJisRzvjArZ%2BpJ9ES13N1zv1m4f9VnfiHOdDEdcZWOUY6xS9Gi27GgC4pLKvWki7aHQISPLz9y0Uo3kGEPO5874RygU9DtXL3P1LY1%2BiVxtw77UEk%3D; \
         WM_NIKE=; \
         JSESSIONID-WYYY=pNCF%2B8xzB2jTGWW7r7JlavTaS0YVMSZBP9THDnXZp86OQ3Aqo5WpW6dr3h6FR3hgevYdmOdO8N7aubiagD%2FhBrf%2BYd%2BcXtBehyUotNH%2BCs%5CqZXKRbf4Pyt6fU1tl7UCsXBvbe6b5%2BQwZ%5Cuth8Shm4fRFdkApHsDIEA9tuUYQYDB7BYuo%3A1543559635032; __utmb=94650624.51.10.1543554354',
        'Host': 'music.163.com',
...
def auto_create_path(FilePath):
    ...
    if os.path.exists(FilePath):   ##目录存在，返回为真
        print( 'dir {} exists'.format(FilePath)  )
        exist = True
    else:
        # 不合法的字符
        FilePath = FilePath.replace("?","").replace("*","").replace("<","").replace(">","")
        os.makedirs(FilePath)
        print("makdir done",FilePath)
    return FilePath, exist

if __name__ == '__main__':
    # 35803个歌手！！！
    singer_dict = get_all_singers_id()
    print("singer",len(singer_dict.keys()))

    # 专辑地址和headers
    singer_id = 3686   # 歌手id，可以在网易云音乐网站上搜索自己喜欢歌手的ID
    for singer_id in list(singer_dict.keys()):
        print("singer_id:",singer_dict[singer_id])
        singer_name = singer_dict[singer_id][0]
        # 存储路径
        #singer_name_obj = re.match(r'./lyrics_(.*)/lyricsList.txt', f_path)
        parent_path = "../result/songs_of_singer_info"
        save_f_path = "{}/lyrics_{}/".format(parent_path,singer_name)
        exist = False
        try:
            _, exist = auto_create_path("\\".join(os.getcwd().split("\\")[:-1]) + save_f_path.replace("..",".") )
            save_f_path = save_f_path.replace("?","").replace("*","").replace("<","").replace(">","")
        except Exception as e:
            print("error",e)
            continue
        if exist:
        ...
        get_singer_songs_info(save_f_path, singer_id=singer_id)
